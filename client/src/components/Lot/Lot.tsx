...
const Lot: React.FC<ILot> = (props: ILot) => {
    ...
    const makeBet = async (lotId: number) => {
        ...
        try {
            const response = await server.makeBet(lotId, bet);
            if (!response) {
                setError("Ошибка при выполнении ставки.");
                return;
            }
            ...
            if (response.error) {
                ...
                switch (errorCode) {
                    case '3012':
                        setError("Нельзя поставить ставку на свой собственный лот.");
                        break;
                    case '3013':
                        setError("У вас уже есть ставка на этот лот. Увеличьте ставку.");
                        break;
                    case '3014':
                        setError("Новая ставка меньше, чем шаг ставки.");
                        break;
                    case '3015':
                        setError("Данный лот уже завершен.");
                        break;
                    case '3016':
                        setError("Лот с данным ID не был найден.");
                        break;
                    case '3017':
                        setError("Недостаточно средств для ставки.");
                        break;
                    default:
                        setError("Произошла ошибка при выполнении ставки.");
                        break;
                }
                return;
            }
            ...
            {lot.status === ELotStatus.open &&
                <div className='lot-bet-panel'>
                    ...
                    <input
                        ...
                        required
                    />
                    {error && <p className="error" id="test-bet-error">{error}</p>}
                </div>}
