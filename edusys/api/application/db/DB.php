...
class DB {
    ...
    public function registration($login, $hash, $name, $family_name, $surname, $group, $status) {
        $this->execute("INSERT INTO users (login,password,name, family_name, surname, class, status) VALUES (?, ?, ?, ?, ?,?)",[$login, $hash, $name, $family_name, $surname, $group, $status]);
    }
    ...
    public function getGroupMembers($class_id) {
        return $this->queryAll('SELECT * FROM users WHERE class_id=?', [$class_id]);
    }
